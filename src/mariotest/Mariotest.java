...
public class Mariotest extends Canvas implements Runnable {
    ...
    gestorEstados = new GestorEstados(this);
    this.addKeyListener(new Teclas(handler, gestorEstados));
    ...
    for (JuegoObjetos obj : new java.util.ArrayList<>(handler.getGameObjs())) {
    ...
    if (gestorNiveles != null && gestorNiveles.permitirMovimientoJugador()) {
        ...
        // respaldo si no hay gestor (no debería pasar)
        handler.tick();
    }
    ...
    if (player.getY() > VENTANA_HEIGHT + 50) {
        System.out.println("[MUERTE] Jugador cayo fuera del mapa (Y=" + player.getY() + ")");       
